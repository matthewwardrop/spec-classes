...
class SpecClassBuilder:
    """
    A class that converts an ordinary class into a spec-class.

    While this class can be used directly as a decorator to mutate incoming class by adding
    helper methods and various dunder methods (like `__init__`, `__repr__`,
    etc), as described in the [documentation](https://matthewwardrop.github.io/spec-classes); it
    is recommended to use the `spec_class` proxy instead (to better satisfy
    ...
    example:
    >>> @SpecClassBuilder(attrs={'my_field'}, key='my_key_field')
    >>> class MyClass:
    >>>     pass
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if len(args) == 1 and isinstance(args[0], type):
            return SpecClassBuilder(**kwargs)(args[0])
    ...
    def __init__(
        ...
        Args:
            ...
            do_not_copy: Whether to avoid copying spec-classes (or attributes
                ...
                methods generated by `spec_class`. If not specified, this will
                default to `False`. Sub-spec-classes will not inherit this
                property, but inherited attributes that have been explicitly
                indicated as `do_not_copy` will maintain this property unless
                overridden.
            frozen: Whether to disallow direct modifications of attributes. If
                not specified, this will default to `False`. Note that `frozen`
                status is not inherited by sub-spec-classes.
    ...
    def bootstrap(self, spec_cls: type):
        ...
        for attr, attr_spec in metadata.attrs.items():
            if attr in attr_types:
                continue
            do_not_copy: bool = attr_spec.do_not_copy
            if isinstance(self.do_not_copy, Iterable) and attr in self.do_not_copy:
                do_not_copy = True
            if attr in spec_cls.__dict__ or do_not_copy is not attr_spec.do_not_copy:
                metadata.attrs[attr] = self.build_attr_spec(
                    ...
                    helpers=attr_spec.helper_methods is not None,
...
spec_class.__signature__ = inspect.Signature(
    parameters=list(_SPEC_CLASS_BUILDER_SIGNATURE.parameters.values())[1:]
)  # Exclude 'self'
