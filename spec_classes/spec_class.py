...
from spec_classes.methods.core import CORE_METHODS, InitMethod, ReprMethod
from cached_property import cached_property
import inspect
from spec_classes.types import keyed
...
from .types import MISSING, Attr, spec_property
...
class spec_class:
    ...
    def __init__(
        self,
        key: str = MISSING,
        shallow_copy: Union[bool, Iterable[str]] = False,
        frozen: bool = False,
        init: bool = True,
        ...
        Args:
            ...
            shallow_copy: Whether to do a shallow or deep copy when doing
    ...
    def bootstrap(self, spec_cls: type):
        ...
        # overridden on class, or the attribute's `shallow_copy` attribute needs
        ...
        for attr, attr_spec in metadata.attrs.items():
            ...
            shallow_copy = (
                self.shallow_copy
                if isinstance(self.shallow_copy, bool)
                else attr in self.shallow_copy
            )
            if attr in spec_cls.__dict__ or shallow_copy is not attr_spec.shallow_copy:
        ...
        for name, member in spec_cls.__dict__.items():
            if name not in metadata.attrs and hasattr(member, "__spec_class_invalidated_by__"):
    ...
    def build_attr_spec(
        self, spec_cls, attr, attr_type, *, shallow_copy=False, owner=None
    ):
        ...
        if hasattr(spec_cls, attr):
            ...
            if isinstance(attr_value, (Attr, dataclasses.Field)):
                setattr(spec_cls, attr, attr_value.default or attr_value.default_factory())
            ...
            attr_spec = Attr.from_attr_value(
                attr, attr_value, type=attr_type, shallow_copy=shallow_copy, owner=owner
            )
        else:
            attr_spec = Attr.from_attr_value(
                attr, MISSING, type=attr_type, shallow_copy=shallow_copy, owner=owner
            )
        ...
        # Check for preparer
        if hasattr(spec_cls, f"_prepare_{attr_spec.name}"):
            attr_spec.prepare = getattr(spec_cls, f"_prepare_{attr_spec.name}")

        # Check for item_preparer
        if attr_spec.is_collection and hasattr(
            spec_cls, f"_prepare_{attr_spec.item_name}"
        ):
            attr_spec.prepare_item = getattr(spec_cls, f"_prepare_{attr_spec.item_name}")
