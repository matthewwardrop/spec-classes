...
class WithMappingItemMethod(AttrMethodDescriptor):
    ...
    @staticmethod
    def with_mapping_item(attr_spec, self, _key=None, _value=None, _inplace=False, _if=True, **attrs):
        if not _if:
            return self
        return mutate_attr(
            obj=self,
            ...
            value=(
                ...
                .add_item(
                    *attr_spec.prepare_item(self, _key, _value, **attrs),  # Tuple of key, value
                    attrs=attrs,
                )
                .collection
            ),
...
class TransformMappingItemMethod(AttrMethodDescriptor):
    ...
    @staticmethod
    def transform_mapping_item(attr_spec, self, _key, _transform, *, _inplace=False, _if=True, **attr_transforms):
        if not _if:
            return self
        return mutate_attr(
            obj=self,
...
class WithoutMappingItemMethod(AttrMethodDescriptor):
    ...
    @staticmethod
    def without_mapping_item(attr_spec, self, _key, *, _inplace=False, _if=True):
        if not _if:
            return self
        return mutate_attr(
            obj=self,
