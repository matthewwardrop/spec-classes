...
class spec_class:
    ...
    def __init__(
            self, *attrs: str, _key: str = None, _skip: Iterable[str] = None, _deepcopy: bool = True,
    ...
    def get_methods_for_spec_class(cls, spec_cls: type) -> Dict[str, Callable]:  # pylint: disable=no-self-argument
        """
        ...
        def init_impl(self, **kwargs):
            for attr in self.__spec_class_annotations__:
                if attr in kwargs:
                    cls._with_attr(self, attr, (copy.deepcopy if cls.spec_deepcopy else lambda x: x)(kwargs[attr]), inplace=True)
                else:
                    ...
                    if cls._check_type(attr_value, self.__spec_class_annotations__[attr]) and not inspect.isfunction(attr_value) and not inspect.isdatadescriptor(attr_value):
                        cls._with_attr(self, attr, (copy.deepcopy if cls.spec_deepcopy else lambda x: x)(attr_value), inplace=True)
                    else:
        ...
        if cls.spec_key:
            key_default = getattr(spec_cls, cls.spec_key, Parameter.empty)
        ...
        methods = {
            '__init__': (
                ...
                .with_arg(
                    cls.spec_key, f"The value to use for the `{cls.spec_key}` key attribute.",
                    default=key_default, annotation=spec_cls.__spec_class_annotations__.get(cls.spec_key),
                    only_if=cls.spec_key
                )
        ...
        return {
            ...
            if cls.spec_cls_methods.get(name, False)
        }
    ...
    def _with_attr(cls, self: Any, name: str, value: Any, inplace: bool = False) -> Any:  # pylint: disable=no-self-argument
        """
        ...
        if value is MISSING:
            return self
        if not inplace:
            self = (copy.deepcopy if cls.spec_deepcopy else copy.copy)(self)
    ...
    # Scalar methods
    def _get_methods_for_scalar(cls, spec_cls: type, attr_name: str, attr_type: Type):  # pylint: disable=no-self-argument
    ...
    def _get_methods_for_list(cls, spec_cls: type, attr_name: str, attr_type: Type) -> Dict[str, Callable]:  # pylint: disable=no-self-argument
        """
    ...
    def _get_methods_for_dict(cls, spec_cls: type, attr_name: str, attr_type: Type) -> Dict[str, Callable]:  # pylint: disable=no-self-argument
        """
    ...
    def _get_methods_for_set(cls, spec_cls: type, attr_name: str, attr_type: Type) -> Dict[str, Callable]:  # pylint: disable=no-self-argument
        """
