...
class TestFramework:
    ...
    def test_spec_methods(self):
        ...
        assert (
            inspect.Signature.from_callable(Item.__init__).parameters["x"].default is MISSING
        )
    ...
    def test_shallow_copy(self):
        @spec_class(shallow_copy=["shallow_list"], bootstrap=True)
        class Item:
            value: str
            deep_list: list
            shallow_list: object

        assert Item.__spec_class__.attrs['value'].shallow_copy is False
        assert Item.__spec_class__.attrs['deep_list'].shallow_copy is False
        assert Item.__spec_class__.attrs['shallow_list'].shallow_copy is True
        ...
        @spec_class(shallow_copy=True, bootstrap=True)
        class ShallowItem:
            value: str
            deep_list: list
            shallow_list: object

        assert ShallowItem.__spec_class__.attrs['value'].shallow_copy is True
        assert ShallowItem.__spec_class__.attrs['deep_list'].shallow_copy is True
        assert ShallowItem.__spec_class__.attrs['shallow_list'].shallow_copy is True
...
class TestSpecAttribute:
    ...
    def test_with(self, spec_cls):
        ...
        with pytest.raises(AttributeError, match=re.escape("`Spec.spec` has not yet been assigned a value.")):
    ...
    def test_illegal_nested_update(self, spec_cls):
        ...
        with pytest.raises(
            TypeError,
            match=re.escape("with_spec() got an unexpected keyword argument 'invalid'"),
        ):
