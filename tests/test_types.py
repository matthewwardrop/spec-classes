...
class TestValidatedType:
    ...
    def test_subclassing(self):
        ...
        with pytest.raises(RuntimeError, match=r"`MyType` is intended to be used as a type annotation, and should not be instantiated\."):
            MyType()
    ...
    def test_bounded(self):
        ...
        with pytest.raises(ValueError, match=r"Can only specify at most one of `gt` or `ge`\."):
            bounded(int, gt=1, ge=1)
        with pytest.raises(ValueError, match=r"Can only specify at most one of `lt` or `le`\."):
